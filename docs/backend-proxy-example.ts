/**
 * =================================================================================
 * ACTION REQUIRED: YOU MUST DEPLOY THIS BACKEND CODE TO FIX CORS ERRORS
 * =================================================================================
 *
 * The `NetworkError` you are seeing is a CORS (Cross-Origin Resource Sharing) issue.
 * It means the backend code currently deployed on Vercel is not correctly
 * responding to the browser's security checks.
 *
 * To fix this, you must replace your existing backend function code with THIS code
 * and then REDEPLOY your application on Vercel.
 *
 * HOW TO DEPLOY:
 * ---------------------
 * 1.  In your project's root, create an `api` directory if it doesn't exist.
 * 2.  Place THIS file's content inside it as `send-email.ts`. The final path
 *     should be `/api/send-email.ts`.
 * 3.  Ensure your Vercel project settings have an Environment Variable:
 *     - Name: `RESEND_API_KEY`
 *     - Value: your_secret_resend_api_key (e.g., re_ZpQj1cxN_...)
 * 4.  Commit the changes and let Vercel redeploy. This new deployment will
 *     contain the correct CORS logic.
 * 5.  Confirm the URL from the Vercel deployment matches the `BACKEND_PROXY_URL`
